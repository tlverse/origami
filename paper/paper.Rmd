...
supported by the `origami` package, for the R language for statistical
computing [@R]. The `origami` R package provides a suite of tools that
...
well as time series methods like _"Rolling Window"_. Formal definitions of these
schemes can be found in [@vanderLaan:2007bz]. It can balance across levels of a
variable (`stratify_ids`), and it can also keep all observations from the same
independent unit together (`cluster`). See the documentation of the `make_folds`
...
For further details about the `origami`, please consult the vignette that
