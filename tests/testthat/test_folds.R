...
test_splits <- function(folds) {
    ...
    test_that("Training and Validation don't overlap", expect_length(all_overlaps, 
        0))
}
...
test_that("V-fold validation sets are exhaustive", expect_equivalent(sort(validation_dt$validation), 
    all_indicies))

max_index_count <- max(table(validation_dt$validation))
test_that("V-fold validation sets are mutually exclusive", expect_equal(max_index_count, 
    1))
...
validation_sets <- cross_validate(get_validation_sets_ids, id_folds, 
    ids)
validation_dt <- validation_sets$fold_data
...
test_that("Each ID only appears in one fold", expect_equal(max_fold_count, 
    1))
...
validation_sets <- cross_validate(get_validation_sets_ids, strata_folds, 
    strata_ids)
validation_dt <- validation_sets$fold_data
one_counts <- validation_dt[, list(one_count = sum(strata_ids)), 
    by = list(fold)]
count_range <- diff(range(one_counts$one_count))
test_that("Strata are roughly balanced", expect_lte(count_range, 
    nfolds))

############################ 

# check ids nested in strata is enforced
test_that("Ids must be nested in strata", expect_error(make_folds(cluster_ids = ids, 
...
test_that("V fold falls back to LOO for small n, large V", expect_length(folds, 
    smalln))
