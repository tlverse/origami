...
test_that("Resub MSE matches simple resub MSE",expect_equal(resub_MSE,simple_resub_MSE,tolerance=0.01))

# cross-validated estimate
folds <- make_folds(mtcars)
results <- cross_validate(cvlm, folds)
cv_MSE=mean(results$SE)
test_that("CV MSE matches previous value",expect_equal(cv_MSE,13.32,tolerance=0.01))
