# Modified version of SL.knn from the SuperLearner package by Eric Polley <eric.polley at nih.gov> Stores X and Y
# in the fit object so they don't have to be provided for new predictions knn{class} will only work with binomial
# to create additional algorithms with different values of k, for example k=20 SL.knn20 <- function(..., k = 20)
# SL.knn(...,k = k)
...
SL.knn_stored <- function(Y, X, newX, family, k = 10, ...) {
    ...
    pred <- (as.numeric(fit.knn) - 1) * attr(fit.knn, "prob") + (1 - (as.numeric(fit.knn) - 1)) * (1 - attr(fit.knn, 
        "prob"))
