# Modified version of SL.knn from the SuperLearner package by Eric Polley
# <eric.polley at nih.gov> Stores X and Y in the fit object so they don't have to
# be provided for new predictions knn{class} will only work with binomial to
# create additional algorithms with different values of k, for example k=20
...
SL.knn_stored <- function(Y, X, newX, family, k = 10, ...) {
    ...
    pred <- (as.numeric(fit.knn) - 1) * attr(fit.knn, "prob") + (1 - (as.numeric(fit.knn) - 
        1)) * (1 - attr(fit.knn, "prob"))
...
predict.SL.knn_stored <- function(object, newdata, ...) {
    .SL.require("class")
    fit.knn <- knn(train = object$X, test = newdata, k = object$k, cl = object$Y, 
        prob = TRUE)
    pred <- (as.numeric(fit.knn) - 1) * attr(fit.knn, "prob") + (1 - (as.numeric(fit.knn) - 
        1)) * (1 - attr(fit.knn, "prob"))
    return(pred)
